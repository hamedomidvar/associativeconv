...
class ResNetBasicblockSep(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, CSG, stride=1, downsample=None):
        super(ResNetBasicblockSep, self).__init__()
        self.stride = stride
        
        
#         self.conv_a = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.conv_dw1 = nn.Conv2d(inplanes,inplanes,3,padding=1,groups=inplanes, bias=False)
        self.conv_pw1 = nn.Conv2d(inplanes,planes,1,stride=stride,padding=0, bias=False)
        
        self.bn_a = nn.BatchNorm2d(planes)


#         self.conv_b = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv_dw2 = nn.Conv2d(planes,planes,3,padding=1,groups=planes, bias=False)
        self.conv_pw2 = nn.Conv2d(planes,planes,1,stride=1,padding=0, bias=False)
...
#         basicblock = self.conv_a(x)
        basicblock = self.conv_dw1(x)
        basicblock = self.conv_pw1(basicblock)
        
        basicblock = self.bn_a(basicblock)
        basicblock = F.relu(basicblock, inplace=True)

#         basicblock = self.conv_b(basicblock)
        basicblock = self.conv_dw2(basicblock)
        basicblock = self.conv_pw2(basicblock)
...
class ResNetBasicblockSep1(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, CSG, stride=1, downsample=None):
        super(ResNetBasicblockSep1, self).__init__()
        self.stride = stride
...
class ResNetBasicblockSep2(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, CSG, stride=1, downsample=None):
        super(ResNetBasicblockSep2, self).__init__()
        self.stride = stride
...
#         self.conv_1_3x3 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.conv_pw1 = nn.Conv2d(3,16,1,stride=1,padding=0, bias=False)
...
def resnet56(num_classes=10):
    ...
    num_classes (uint): number of classes
    """
    model = CifarResNet(ResNetBasicblockSep1, 56, num_classes)
    return model
